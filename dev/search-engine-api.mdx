---
title: "Search Engine API: Adding Custom Book Sources"
sidebarTitle: Search Engine API
description: Learn how the SearchEngine interface works and how to wire up a new book data source — from implementing the right sub-interface to registering it in EngineId.
---
...
<Steps>
  <Step title="Implement the SearchEngine interface">
    ...
    <Note>
    The `fetchCovers` parameter is a boolean array of length `DBKey.NR_OF_BOOK_COVERS` (currently 2). Index 0 is the front cover, index 1 is the back cover. Only download cover images for indexes where `fetchCovers[i]` is `true`.
    </Note>
  </Step>
